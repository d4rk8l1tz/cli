...
func TestInstallGitHook_WorktreeInstallsInCommonHooks(t *testing.T) {
	...
	count, err := InstallGitHook(context.Background(), true)
	if err != nil {
...
func TestInstallGitHook_Idempotent(t *testing.T) {
	...
	firstCount, err := InstallGitHook(context.Background(), true)
	if err != nil {
	...
	secondCount, err := InstallGitHook(context.Background(), true)
	if err != nil {
...
func TestRemoveGitHook_CoreHooksPathRelative(t *testing.T) {
	...
	installCount, err := InstallGitHook(context.Background(), true)
	if err != nil {
		t.Fatalf("InstallGitHook() error = %v", err)
	}
	if installCount == 0 {
...
func TestInstallGitHook_DoesNotOverwriteExistingBackup(t *testing.T) {
	...
	_, err := InstallGitHook(context.Background(), true)
	if err != nil {
		t.Fatalf("InstallGitHook() error = %v", err)
	}
